...
class InterfaceMonitor:
    ...
    def __init__(self, name: str, client, firewall_config=None):
        ...
        self.vendor_type = getattr(firewall_config, 'type', 'palo_alto') if firewall_config else 'palo_alto'
        ...
        try:
            ...
            if self.vendor_type == 'palo_alto':
                ...
                if hasattr(self.client, 'collect_interface_stats'):
                    ...
                    LOG.warning(f"{self.name}: FortinetClient missing collect_interface_stats method")
                    return False
            else:
                LOG.debug(f"{self.name}: Interface collection not implemented for {self.vendor_type}")
                return False
