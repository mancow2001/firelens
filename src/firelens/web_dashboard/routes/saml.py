...
async def saml_login(request: Request):
    ...
    try:
        ...
        return templates.TemplateResponse("admin_saml_error.html", {
            "request": request,
...
async def saml_slo(request: Request):
    ...
    try:
        ...
        success, redirect_url, error = saml_handler.process_logout(
            request_data,
            delete_session_callback=delete_session_callback
        )
...
async def admin_saml_page(request: Request):
    ...
    return templates.TemplateResponse("admin_saml.html", {
        "request": request,
        "user": user,
...
async def admin_api_save_saml_config(request: Request):
    ...
    try:
        ...
        if not hasattr(config_manager.global_config, 'admin') or not config_manager.global_config.admin:
    ...
    try:
        ...
        if data.get('enabled'):
            ...
            if cert and '-----BEGIN CERTIFICATE-----' not in cert:
                errors.append("IdP Certificate should be in PEM format (-----BEGIN CERTIFICATE-----)")
        ...
        return JSONResponse({
            "valid": True,
            "message": "Configuration valid (SAML disabled)"
        })
